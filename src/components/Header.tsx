...
const Header: React.FC = () => {
  ...
  const showSecureCheckout = location.pathname === "/checkout" || location.pathname === '/payment';
  ...
  const isCartEmpty = productsInCartCount === 0;
    ...
    <header className="header">
...

      {
        showSecureCheckout ? (
          ...
          <Link to='/checkout' className={`header__cart ${isCartEmpty ? 'disabled' : ''}`} style={ isCartEmpty ? { pointerEvents: 'none'} : {}}>
